...
from claude_hub.services import qa_session
...
@router.post("/{project_id}/qa/start")
async def start_qa(project_id: str):
    """Start the QA Agent session for a project."""
    project, gh_token = await _get_project_and_token(project_id)
    try:
        session_name = qa_session.start_qa_session(project, gh_token)
    except Exception as e:
        logger.error("Failed to start QA Agent for %s: %s", project_id, e)
        raise HTTPException(500, f"Failed to start QA Agent: {e}")
    return {"session_name": session_name, **qa_session.get_status(project_id)}


@router.post("/{project_id}/qa/restart")
async def restart_qa(project_id: str):
    """Restart the QA Agent session."""
    project, gh_token = await _get_project_and_token(project_id)
    try:
        session_name = qa_session.restart_qa_session(project, gh_token)
    except Exception as e:
        logger.error("Failed to restart QA Agent for %s: %s", project_id, e)
        raise HTTPException(500, f"Failed to restart QA Agent: {e}")
    return {"session_name": session_name, **qa_session.get_status(project_id)}


@router.get("/{project_id}/qa/status")
async def qa_status(project_id: str):
    """Get QA Agent session status."""
    project = await redis_client.get_project(project_id)
    if not project:
        raise HTTPException(404, "Project not found")
    return qa_session.get_status(project_id)


@ws_router.websocket("/ws/kanban/{project_id}/qa-terminal")
async def qa_terminal(websocket: WebSocket, project_id: str, token: str = Query(default="")):
    """WebSocket endpoint that bridges xterm.js to the QA Agent tmux session."""
    ...
    # Auto-start QA Agent if not running (requires kanban to be running first)
    if not qa_session.is_alive(project_id):
        if not kanban_manager.is_alive(project_id):
            await websocket.close(code=4003, reason="Kanban session not running")
            return
        try:
            gh_token = project.get("gh_token", "")
            qa_session.start_qa_session(project, gh_token)
            await asyncio.sleep(1)
        except Exception as e:
            logger.error("Failed to auto-start QA Agent for %s: %s", project_id, e)
            await websocket.close(code=4003, reason=f"Failed to start QA Agent: {e}")
            return

    session_name = qa_session._session_name(project_id)
    await websocket.accept()
    logger.info("QA Agent terminal connected for project %s", project_id)
    ...
    process = await asyncio.create_subprocess_exec(
        ...
        preexec_fn=_child_setup, env=env,
    )
    os.close(slave_fd)
                ...
                if "bytes" in msg:
                    ...
                    if data and data[0:1] == b"\x01":
                        try:
                            ...
                            _set_pty_size(master_fd, resize.get("cols", 80), resize.get("rows", 24))
                            if child_pid:
                                try:
    ...
    try:
        ...
        try:
            ...
            pass
        logger.info("QA Agent terminal disconnected for project %s", project_id)
